...
## 実験結果（速報）
初期実験の結果、**平易化により性能が低下**することが判明しました（詳細は `results/Result.md` を参照）：

- **Baseline (Original)**: Macro-F1: 0.719, Accuracy: 0.720
- **Simplification**: Macro-F1: 0.675, Accuracy: 0.681（約4ポイント低下）
- **Concat**: Macro-F1: 0.697, Accuracy: 0.695（中間的な性能）

平易化テキストの品質評価（BERTScore）では平均0.840と高い意味的類似度を示しており、平易化自体の品質は保たれています。しかし、感情分類において重要な微妙なニュアンス（皮肉、含意、程度副詞など）が失われている可能性が示唆されています。
...
- **品質評価** (`src/compute_bertscore.py`): BERTScoreによる意味的類似度の計測と統計分析
...
```bash
# BERTScoreによる評価
poetry run python src/compute_bertscore.py data/wrime_simp_train.pkl \
  --model tohoku-nlp/bert-base-japanese-v3 \
  --lang ja \
  --device cuda \
  --batch_size 8 \
  --out_prefix results/simplification_bertscore/bertscore_output
```
- 平易化文と原文の意味的類似度を計測
- 出力: BERTScore付きpkl/csv、ヒストグラム、統計情報
...
- **品質評価**:
  1. **BERTScore** による意味保持チェック（✅ 実装済み）
     - 平均スコア: 0.840（中央値: 0.846）
     - 高い意味的類似度を確認
  2. **Cosine類似度** による意味的一致性評価（🚧 計画中）
  3. 原文と平易化文での極性不一致サンプルの検出（🚧 計画中）
...
## 評価指標と分析方法
- **主評価指標**：Macro-F1（✅ 実装済み）
- **補助指標**：Accuracy、クラス別 F1、混同行列（✅ 実装済み）
- **意味保持評価**：BERTScore（✅ 実装済み）
...
### 確認された結果
1. **平易化による性能低下**: 初期実験では、平易化により全体的な性能が約4ポイント低下（Macro-F1: 0.719 → 0.675）
2. **Neutralクラスへの影響**: 特にneutralクラスでF1スコアが大きく低下（0.599 → 0.528）
3. **意味保持の成功**: BERTScoreによる評価では高い意味的類似度（平均0.840）を維持
4. **データ拡張効果の限定性**: Concat条件は性能向上に寄与せず（Macro-F1: 0.697）

### 示唆される考察
1. 平易化は意味保持に成功しているものの、感情分類に必要な微妙なニュアンス（皮肉、含意、程度副詞など）が失われている可能性
2. 特にneutralクラスのような曖昧な感情の識別において、元の表現の複雑さが重要な手がかりとなっている
3. Concat条件での性能がベースラインに達しないことから、平易化文がノイズとして作用している可能性
4. Noise-norm条件の実装により、簡約効果が表記正規化ではなく内容変化によるものかを今後検証できる
...
- ✅ データセット読み込みと3クラス変換
- ✅ サブセット作成機能（30%サンプリング）
- ✅ ローカルLLMによる平易化（Swallow-7b-instruct-v0.1）
- ✅ API平易化検証スクリプト（OpenAI/Anthropic/Google）
- ✅ BERTScoreによる品質評価
- ✅ BERT分類モデルの実装（東北大学版日本語BERT）
- ✅ 学習・評価パイプライン（Macro-F1、Accuracy、混同行列）
- ✅ 複数条件対応（orig/simp/concat）
- ✅ 初期実験の実行と結果分析

### 今後のタスク
- 🚧 Noise-norm条件の実装
- 🚧 拡張品質評価パイプライン（Cosine類似度、chrF）
- 🚧 複数シードでの実験と統計的有意性検定
- 🚧 層別分析（文長、語彙難易度、ノイズレベル別）
- 🚧 失敗例解析（平易化で性能が落ちたケースの調査）
- 🚧 自動再平易化パイプラインの構築
